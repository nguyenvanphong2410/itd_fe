...
import { setOpenModalDocumentOfName } from '../../../../../states/modules/document';
...
const ModalDocumentOfName = ({ nameClick, listDocumentOfName }) => {
  ...
  return (
    <>
      ...
      <Modal
        title={
          <span className={styles.titleModal}>Tài liệu của <span className={styles.name}> {nameClick}</span> </span>
        }
      ...
      >
        ...
        <span className={styles.totalDocument}>Tổng số tài liệu của {nameClick} là {documents?.length} tài liệu</span>
          {
            documents?.length > 0 ?
...
export default ModalDocumentOfName;